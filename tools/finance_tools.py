...
def load_and_categorize_statement(user_id: int, csv_path: str, replace_existing: bool = False) -> dict:
    ...
    if db.get_all_transactions(user_id) and not replace_existing:
        return {"status": "error", "error": "Transactions already exist for this account. Review the import before replacing existing data."}
    if replace_existing:
